...
| Compromised container "calls home" to an external C2 server (e.g. `models.litellm.cloud`) | `harden-egress.sh` — firewalld policy scoped to the full LiteLLM bridge subnet: allow internal traffic, DNS, IMDS, and explicitly configured Bedrock/USAI/Codex/STS/RDS destinations; reject all other outbound. It fails closed if an enabled external broker lacks a CIDR allowlist. | **Script ready; needs network/AWS architect input to run it** — public USAI/ChatGPT/CDN CIDRs can rotate, so a controlled egress proxy or private endpoint is preferred. |
