...
| Bedrock destination is an open question (VPC endpoint vs public IP allow-list). | Security Group `LitellmProxySecurityGroup` in `ecs-security-groups.yaml` allows outbound TCP 443 only to the Bedrock VPC endpoint CIDR (or fallback public ranges). |

**Action**: Deploy `ecs-security-groups.yaml` via CloudFormation. Set `BedrockEndpointCidr` to your VPC endpoint ENI CIDR (preferred) or `BedrockPublicCidrs` to the published ranges. The `LitellmDbSecurityGroup` has **zero egress** — Postgres cannot call home.
